...
IMAPClient.prototype._formatEnvelope = function(envelopeData){
    ...
    if (dataObject["RFC822.HEADER"]){
        ...
        var msgIdStartIndex = dataObject["RFC822.HEADER"].indexOf('Message-ID:');
        if (msgIdStartIndex != -1) {
            msgIdStartIndex = dataObject["RFC822.HEADER"].indexOf('<', msgIdStartIndex);
            var msgIdEndIndex = dataObject["RFC822.HEADER"].indexOf('>', msgIdStartIndex);
            message.messageId = dataObject["RFC822.HEADER"].substring(msgIdStartIndex, msgIdEndIndex + 1);
        }
    }
