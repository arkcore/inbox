...
function IMAPClient(port, host, options){
     ...
     * tls options
     */
    this.sslOptions =  this.options.sslOptions || { rejectUnauthorized: true };
 ...
 * Socket timeout
 */
IMAPClient.prototype.SOCKET_TIMEOUT = 120 * 1000;
...
IMAPClient.prototype.connect = function () {
    this._id = String(++connectionNumber);

    if (this.options.secureConnection) {
        this._connection = tls.connect(this.port, this.host, this.sslOptions, this._onConnect.bind(this));
    } else {
...
IMAPClient.prototype._onConnect = function () {
    ...
    if ('setKeepAlive' in this._connection) {
        ...
        this._connection.setTimeout(this.SOCKET_TIMEOUT, this._onSocketTimeout.bind(this));
        ...
        this._connection.socket.setTimeout(this.SOCKET_TIMEOUT, this._onSocketTimeout.bind(this));
    }
...
/**
 * 'error' emits socket timeout, will severe connection after that and try reconnecting
 *
 *  @event 'timeout'
 *
 */
IMAPClient.prototype._onSocketTimeout = function () {
    this.emit('error', new Error('ESOCKTIMEOUT ' + this._id));
};
        ...
        if(!isNaN(data[1])) {
            ...
            switch (data[2]) {
                ...
                case "EXPUNGE":
                    ...
                    this._handlerUntaggedExpunge(data);
                    return;
...
IMAPClient.prototype._processCommandQueue = function () {
    ...
    try {
      ...
      console.error('Writing to a closed socket:', e);
      return this._close();
    }
    ...
    this._currentRequest = {
        ...
        tag: command.tag,
        callback: function mainLoopCallback(status, params) {

            clearTimeout(self._shouldIdleTimer);
            clearTimeout(self._idleTimer);
            if (!self.idling && !self._idleWait && self._currentState == self.states.SELECTED) {
                self._shouldIdleTimer = setTimeout(self.idle.bind(self), self.ENTER_IDLE);
            }

            if (hasCallback) {
                command.callback.call(self, status, params);
            }
            self._currentRequest = false;

            // make async
            setImmediate(function () {
              self._processCommandQueue.call(self);
            });
        }
    };
};
...
IMAPClient.prototype._handlerTaggedCondstore = function(){
    ...
    if (this._capabilities.indexOf('ID') >= 0) {
        if (clientData.length) {
            this._send("ID (" + clientData + ")", this._handlerTaggedId);
        } else {
            this._send("ID NIL", this._handlerTaggedId);
        }
    } else {
        this._postReady();
    }
...
/**
 * Handle untagged EXPUNGE responses, emits sequence number
 *
 * @param {Array} list
 */
IMAPClient.prototype._handlerUntaggedExpunge = function(list){
    var seqno = list[1];
...
/**
 * Handle untagged SEARCH responses, this is a list of seq or uid values
 *
 * @param {Array} list Params about a message
 */
    ...
    if(this._currentState == this.states.PREAUTH){
        ...
        if(this._capabilities.indexOf("AUTH=XOAUTH2")>=0 && this._xoauth2){
            ...
            this._send("LOGIN "+this._escapeString(this.options.auth.user)+" "+
                this._escapeString(this.options.auth.pass), this._handlerTaggedLogin);
        }
...
IMAPClient.prototype._postAuth = function(){
    if(this._capabilities.indexOf("CONDSTORE")>=0){
        this._send("ENABLE CONDSTORE", this._handlerTaggedCondstore);
    }else{
...
IMAPClient.prototype._formatEnvelope = function(envelopeData){
    ...
    for(i=0, len = envelopeData.length; i<len; i++){
        if(!lastKey){
            envDatum = envelopeData[i];
            lastKey = (envDatum && envDatum.value || envDatum || "").toString();
        }else{
    ...
    var headerObj = dataObject["RFC822.HEADER"];
    if (headerObj){

        var startIndex = headerObj.indexOf('References:') + 'References:'.length;
        if (startIndex != -1){
            var endIndex = headerObj.indexOf(':', startIndex);
            message.references = headerObj.substring(startIndex, endIndex).match(/<(.*?)>+/igm);
        }

        var temp = headerObj.match(/(X-Originating-IP:).+?]/igm);
        if (temp){
            message.xOriginatingIP = temp[0].substring('X-Originating-IP:'.length).trim().replace(/(\[|\])/g, '');
        }

        var msgId = /Message-Id:[ ]*(<[^>]+>)/igm.exec(headerObj);
        if (msgId) {
            message.messageId = msgId[1];
        }
    }
        ...
        message.folders = (dataObject["X-GM-LABELS"] || []).map(function (label) {
            ...
            if(label && typeof label == "object" && label.params){
                label = label.params.map(function(localLabel){
                    ...
                    return localLabel;
                }, this).join(this._mailboxDelimiter);
            }
...
IMAPClient.prototype._formatEnvelopeAddress = function(address){
    ...
    try {
        ...
        // { value: 'narcisakaminski@gmail.com\\" NIL ',
        // partial: [ 'narcisakaminski"', 'gmail.com' ] }
...
/**
 * Lists changes in the given mailbox
 * @param {Number}   modseq   - only list changes after this sequence
 * @param {Function} callback - <err, messages>
 */
IMAPClient.prototype.listMessageChanges = function (modseq, callback) {
    this.fetch('1:*',
        'UID FLAGS ENVELOPE' + (this._capabilities.indexOf('X-GM-EXT-1') >= 0 ? ' X-GM-LABELS' : ''),
        this._capabilities.indexOf('CONDSTORE') >= 0 ? 'CHANGEDSINCE ' + modseq : '',
        callback
    );
};

/**
 * Returns seq to { uid, messageId } for a given mailbox
 * @param {Function} callback - <err, messages>
 */
IMAPClient.prototype.fetchAll = function (callback) {
    this.fetch('1:*', 'UID ENVELOPE', false, callback);
};

/**
 * Fetches new messages that were received after last seq update
 * @param {Number}   lastSeenUID
 * @param {Number}   modseq
 * @param {Function} callback    <err, messages>
 */
IMAPClient.prototype.fetchNewMessages = function (lastSeenUID, modseq, callback) {
    this.fetch(lastSeenUID + ':*',
      'UID FLAGS ENVELOPE' + (this._capabilities.indexOf('X-GM-EXT-1') >= 0 ? ' X-GM-LABELS' : ''),
      modseq ? 'CHANGEDSINCE ' + modseq : '',
      callback
    );
};

// implement end method for standard-reconnect
IMAPClient.prototype.end = function () {
    clearTimeout(this._shouldIdleTimer);
    clearTimeout(this._idleTimer);
    clearTimeout(this._greetingTimeout);
    ...
    // mute future errors, we will dispose of it
    this.on('error', _.noop);

    // this will also be cleaned
...
IMAPClient.prototype.listMailboxes = function(path, all, callback){
    this._rootMailbox.listAllChildren(path, all, callback);
};
...
IMAPClient.prototype.openMailbox = function (path, options, callback) {
    ...
    var mbox = this.getCurrentMailbox();
    var isReadOnly = !!options.readOnly;
    if (mbox && mbox.path === path && mbox.isReadOnly === isReadOnly && !options.force) {
        return setImmediate(callback, null, mbox);
    }

    this._send(command + ' ' + this._escapeString(path) + (this._condstoreEnabled ? ' (CONDSTORE)' : ''),

        function (arg1, arg2) {
            this._handlerTaggedSelect(callback, arg1, arg2);
        },
...
IMAPClient.prototype.listFlags = function(from, limit, callback){
    ...
    this._send("FETCH " + from + ":" + to + " (UID FLAGS)",
...
/**
 * This method doesnt check that the original folder is the same as the current one, therefore
 * we must implement it
 */
IMAPClient.prototype.createMessageStream = function (uid, path) {
    ...
    var readOnly;
    if (!path) {
        path = this.getCurrentMailbox().path;
        readOnly = this.getCurrentMailbox().readOnly;
    } else {
        readOnly = true;
    }

    // make sure that the mailbox is the same when we open the message
    this.openMailbox(path, { readOnly: readOnly }, _.noop);
...
/**
 * General seq fetch function
 * @param  {String}   uid
 * @param  {String}   attributes
 * @param  {String}   extra
 * @param  {Function} callback   <err, maillist>
 */
IMAPClient.prototype.fetch = function (uid, attributes, extra, callback) {
    ...
    var command = 'FETCH ' + uid + ' (' + attributes + ')' + (extra ? ' (' + extra + ')' : '');
    ...
    this._send(command,
        ...
        function (status) {
            ...
            if (status === 'OK') {
                ...
                this._mailList = [];
                callback(null, maillist);
            } else {
                callback(new Error('Error fetching list: ' + command));
            }
        },
...
IMAPClient.prototype.deleteMessage = function (uid, callback) {
    ...
    this.addFlags(uid, '\\Deleted', function (error) {
        ...
        self._send('EXPUNGE', function (status) {
            if(status !== 'OK'){
...
IMAPClient.prototype.moveMessage = function (uid, destination, callback) {
    ...
    this.copyMessage(uid, destination, function (error) {
        ...
        self.deleteMessage(uid, function (error) {
 ...
 */
IMAPClient.prototype.storeMessage = function (message, flags, date, callback) {
    ...
    if (date) {
        if (!isDate(date)) {
            return setImmediate(callback, new Error('`date` is not a Date object'));
        }
        ...
        date = [
            ...
            date.getFullYear(),
            ' ',
            ('0' + options.date.getHours()).slice(-2),
            ':',
            ('0' + options.date.getSeconds()).slice(-2),
            ((options.date.getTimezoneOffset() > 0) ? ' -' : ' +' ),
            ('0' + (-options.date.getTimezoneOffset() / 60)).slice(-2),
            ('0' + (-options.date.getTimezoneOffset() % 60)).slice(-2),
            '""'
        ].join('');
    } else {
    ...
    this._send("APPEND " + this._escapeString(this._selectedMailbox.path) + flags + date + " {" + message.length+"}", function (status, data) {
        this._literalHandler = null;
        if(status == "OK"){
...
IMAPClient.prototype.search = function(query, isUid, callback){
    ...
    var queryType = isUid ? "UID SEARCH" : "SEARCH",
        queryTerm = buildTerm.call(this, query);
...
function escapeDate(date) {
    return this._escapeString(date.toUTCString().replace(/^\w+, (\d+) (\w+) (\d+).*/, "$1-$2-$3"));
}
...
function escapeParam(param) {
    ...
    if (typeof param == "number") {
        ...
        list.push(this._escapeString(param));

    } else if (Object.prototype.toString.call(param) == "[object Date]") {

        list.push(escapeDate.call(this, param));

    } else if (Array.isArray(param)) {

        param.map(escapeParam, this).forEach(function (p) {
            if (typeof p == "string"){
                list.push(p);
            }
        });
        ...
        return buildTerm.call(this, param);
    }
...
function buildTerm(query) {
    ...
    return Object.keys(query).map(function (key) {
        ...
        [].concat(query[key] || []).forEach(function (param) {
            param = escapeParam.call(self, param);
            if (param) {
                params.push(param);
            }
        });
